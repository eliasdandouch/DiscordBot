...
class MinecraftServer(commands.Cog):
    ...
    async def mc(self,ctx,password: str = os.environ['PASSWORD'],*,mcserver: str = '1.1.1.1'):
        if password == os.environ['PASSWORD']:
            await ctx.author.send(f"{ctx.message.author.mention}, Congratulations! You've entered the right password")
            await ctx.author.send(f"{ctx.message.author.mention}, the IP to the sever is {mcserver}")
