...
class MinecraftServer(commands.Cog):
    ...
    async def mc(self,ctx,password: str = os.environ['PASSWORD'],server: str = None,*,vanillaserver: str = os.environ['VANILLASERVER'],ftbserver: str = os.environ['FTBSERVER']):
        if password == os.environ['PASSWORD'] and server == 'vanilla':
            ...
            print(f"{ctx.message.author.mention}, has ran $mcip successfully using vanilla")
            ...
            await ctx.message.delete()
            print(f"{ctx.message.author.mention}, has ran $mcip successfully using ftb")
            ...
            await ctx.message.delete()
            print(f"{ctx.message.author.mention}, has ran $mcip but has typed the password incorrectly")
        elif server == None:
            ...
            await ctx.message.delete()
            print(f"{ctx.message.author.mention}, has ran $mcip but failed to pass a server")
        else:
            return
